...
Future<Map<String, dynamic>> runPythonEcbTest(Map<String, dynamic> input) async {
  ...
  try {
    final result = await Process.run(
      'python3',
      ['test/comparison_tests/python_scripts/test_ecb_crypto.py', 'test_ecb_input.json'],
    );
...
void main() {
  group('ECB Python Comparison Tests', () {
    test('ECB encryption matches Python', () async {
      ...
      final dartCiphertext = encrypted.map((b) => b.toRadixString(16).padLeft(2, '0')).join('');

      print('Python ciphertext: $pythonCiphertext');
      print('Dart   ciphertext: $dartCiphertext');

      expect(dartCiphertext, equals(pythonCiphertext),
    ...
    test('ECB decryption matches Python', () async {
      ...
      expect(plaintext, equals(pythonResult['plaintext']),
    ...
    test('ECB roundtrip matches Python', () async {
      ...
      expect(pythonResult['match'], isTrue,
             reason: 'Python roundtrip should succeed');
      expect(result, equals(testInput['plaintext']),
             reason: 'Dart roundtrip should succeed');
      expect(result, equals(pythonResult['decrypted_plaintext']),
    ...
    test('Multiple test vectors match Python', () async {
      final testVectors = [
        ...
        {'key': 'key3_zyxwvutsrqpo', 'plaintext': 'A longer message that spans multiple blocks of encryption data'},
      ];
